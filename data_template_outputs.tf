...
data "template_file" "s3_full_user_output" {
  ...
  vars = {
    ...
    secret-key = element(
      aws_iam_access_key.iam_user_s3_full_access.*.encrypted_secret,
      count.index,
    )
    password = element(
      aws_iam_user_login_profile.s3_full_login.*.encrypted_password,
      count.index,
    )
  }
}
...
data "template_file" "s3_list_delete_user_output" {
  ...
  vars = {
    ...
    access-key = element(
      aws_iam_access_key.iam_user_s3_list_delete_access.*.id,
      count.index,
    )
    secret-key = element(
      aws_iam_access_key.iam_user_s3_list_delete_access.*.encrypted_secret,
      count.index,
    )
    password = element(
      aws_iam_user_login_profile.s3_list_delete_login.*.encrypted_password,
      count.index,
    )
  }
}
...
data "template_file" "s3_get_delete_user_output" {
  ...
  vars = {
    ...
    access-key = element(
      aws_iam_access_key.iam_user_s3_get_delete_access.*.id,
      count.index,
    )
    secret-key = element(
      aws_iam_access_key.iam_user_s3_get_delete_access.*.encrypted_secret,
      count.index,
    )
  }
}
...
data "template_file" "standard_user_output" {
  ...
  vars = {
    ...
    access-key = element(
      aws_iam_access_key.iam_user_standard_access.*.id,
      count.index,
    )
    secret-key = element(
      aws_iam_access_key.iam_user_standard_access.*.encrypted_secret,
      count.index,
    )
